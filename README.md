...
*   Intelligently constructs the prompt using your story's full context (main text, memory, world info, and author's notes).
...
2.  Install the script by [clicking here](https://github.com/LordFoogThe4rd/mikupad-hypebot/raw/refs/heads/main/mikupad_hypebot.user.js). Your userscript manager should intercept this and ask for confirmation.
...
*   **Model**: The identifier for the model you wish to use.
...
*   **Reply Regex**: A regular expression used to extract the desired comment from the AI's full response. This is useful if the model wraps its output in extra text or complex JSON.
    *   *Default*: `(?<="comment"\s*:\s*")(.*?)(?=")` (Extracts the value of the "comment" field in a JSON object).
...
The script periodically checks if you have written new text. If new text is detected, it intelligently assembles the full prompt context—mimicking Mikupad's own logic by including memory, world info, and author's notes—and sends a slice of the most recent content to the specified AI model along with the system prompt. It then uses the `Reply Regex` to parse the AI's response and displays the extracted comment in the Hypebot UI panel.
